...
abstract class Button{
	...
	// abstract 메서드는 상속 받아서 써야하기 때문에 private으로 지정할 수 없다.
...
class CalcButton extends Button{
	...
	protected void onclick() {
		try {
			...
			Process pro = rt.exec(cmd);			// 실행우 프로세스를 저장해두었다가
			Thread.sleep(2000);					// 2초후
			pro.destroy();						// 프로그램을 종료한다
...
//		btn3.click();
		...
		CalcButton btn4 = new CalcButton();	// 서브 클래스의 객체를 슈퍼 클래스로 참조한다
		btn4.click();
		// 추상 클래스가 직접 객체를 생성할 수 없지만, 자료형으로써 참조변수를 사용하는 것은 가능하다
			
		Button btn5 = new Button() {	// 익명 클래스
			// 클래스 문법이라 private 적용됌
