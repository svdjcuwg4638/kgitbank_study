...
<table>
	...
	<tr>
		...
		<td>특정 이름으로 전달된 여러 값을 String[] 형태로 변환한다</td>
		<td><%=request.getParameterValues("name") %>
	</tr>
	<tr>
		<td>getRemoteAddr()</td>
	...
	<tr>
		<td>getLocalPort()</td>
		<td>서버가 운용중인 포트의 번호를 int형으로 변환한다</td>
		<td><%=request.getLocalPort() %></td>
	</tr>
	<tr>
		<td>getRequestURI()</td>
		<td>요청받은 지원(문서)의 경로를 문자열로 반환한다</td>
	...
	<tr>
		...
		<td>요청 헤더에 담긴 특정 요소를 문자열 현태로 반환한다(referer는 참조자, 어디에서 왔는가)</td>
