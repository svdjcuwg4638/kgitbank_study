...
public class Ex01 {
	public static void main(String[] args) throws Exception {
		...
		// 한글은 처리할수 없으므로, 2바이트 입력을 처리할 수 있는 새로운 함수를 사용하게 된다
		// 1바이트 입력을 처리하는 객체를 재료로 사용하여 2바이트를 처리하는 새로운 객체를 생성한다
		InputStreamReader isr = new InputStreamReader(System.in);
		
		System.out.printf("한글 하나 입력 : ");
		int num2 = isr.read();
		...
		while(sc2.hasNextLine()) {				//파일에 읽어내지 않은 다음줄이 있으면
			String line = sc2.nextLine();		// 한줄을 읽어내서 (\n이전까지)
